...
interface PlayerControllerProps {
  ...
  disabled: boolean;
  /** Called each frame with heading angle so camera can use it */
  onHeadBob?: (bob: number) => void;
}

export default function PlayerController({ myPos, yaw, keys, disabled, onHeadBob }: PlayerControllerProps) {
  const velocity = useRef(new THREE.Vector2(0, 0)); // xz velocity
  ...
  useFrame((_, delta) => {
    ...
    const targetSpeed = k.sprint ? SPRINT_SPEED : WALK_SPEED;
    ...
    if (hasInput) {
      ...
      facingAngle.current = Math.atan2(inputX, inputZ);
    } else {
      // Decelerate
    ...
    // Boundary + obstacle collision (slide along walls)
    ...
    if (hasInput && isGrounded.current && speed > 0.02) {
      bobPhase.current += dt * HEAD_BOB_SPEED * (k.sprint ? 1.4 : 1);
      const bob = Math.sin(bobPhase.current) * HEAD_BOB_AMOUNT * speed * 10;
      onHeadBob?.(bob);
    } else {
      bobPhase.current = 0;
      onHeadBob?.(0);
